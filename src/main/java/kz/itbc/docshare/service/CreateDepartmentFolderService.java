...
public class CreateDepartmentFolderService implements Service {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        int departmentID = Integer.parseInt(req.getParameter(DEPARTMENT_ID_PARAMETER));
        FolderDAO folderDAO = new FolderDAO();
        Map<String, String> message = new HashMap<>();
        boolean flagHidden = Boolean.parseBoolean(req.getParameter(FLAG_HIDDEN_PARAMETER));

        try {
            if(folderDAO.departmentFolderExists(departmentID, folder_name)){
                message.put(ERROR_ATTRIBUTE, "Папка с таким названием уже существует");
            } else{
                try {
                    new FolderDAO().addDepartmentFolder(folder_name, departmentID, flagHidden, user);
